...
RSpec.describe "book_index", type: :feature do
  before :each do
    ...
    @book_5 = @jim.books.create(title: "Light My Fire", pages: 26, year: 1967, cover_image: "https://m.media-amazon.com/images/M/MV5BN2EyZjM3NzUtNWUzMi00MTgxLWI0NTctMzY4M2VlOTdjZWRiXkEyXkFqcGdeQXVyNDUzOTQ5MjY@._V1_.jpg")
    # review_1 = book_1.reviews.create(title: "Review book 1", rating: 5, user: "John", description: "This book was upsetting." )
    @review_2 = @book_1.reviews.create(title: "Review book 1", rating: 1, user: "Jennica", description: "This book was pretty cool, but I wish it had more violence." )
    @review_3 = @book_1.reviews.create(title: "Review book 1", rating: 1, user: "Bob", description: "This book was boring." )
    @review_4 = @book_1.reviews.create(title: "Review book 1", rating: 1, user: "Larold", description: "This book was boring." )
    @review_5 = @book_2.reviews.create(title: "Review book 2", rating: 2, user: "Frank", description: "This book was rad." )
    @review_6 = @book_2.reviews.create(title: "Review book 2", rating: 2, user: "Willy Wonka", description: "This book was stupid, but I would read it again." )
    @review_7 = @book_2.reviews.create(title: "Review book 2", rating: 2, user: "David", description: "This book was my favorite." )
    @review_8 = @book_2.reviews.create(title: "Review book 2", rating: 2, user: "Donald Trump", description: "This book was not as good as Art of the deal." )
    @review_9 = @book_3.reviews.create(title: "Review book 3", rating: 3, user: "David", description: "This book was a journey into the depths of my soul." )
    @review_10 = @book_3.reviews.create(title: "Review book 3", rating: 2, user: "Jennica", description: "Dont read this book to your children." )
    @review_11 = @book_3.reviews.create(title: "Review book 3", rating: 4, user: "David", description: "A thrilling experience." )
    @review_12 = @book_3.reviews.create(title: "Review book 3", rating: 3, user: "David", description: "not A thrilling experience." )
    @review_13 = @book_3.reviews.create(title: "Review book 3", rating: 3, user: "David", description: "not A thrilling experience." )
    @review_14 = @book_4.reviews.create(title: "Review book 4", rating: 5, user: "Joan", description: "yes, it was worth my time" )
    @review_15 = @book_4.reviews.create(title: "Review book 4", rating: 5, user: "Joan", description: "no it was not worth my time" )
    @review_16 = @book_5.reviews.create(title: "Review book 5", rating: 4, user: "Joan", description: "loved it" )
  end
  ...
  it 'shows book statistics' do
    ...
    visit books_path
      ...
      within("#book-#{@book_1.id}") do
        expect(page).to have_content("Average Rating: 1.0")
    ...
    it 'When I visit the book index page i can sort' do
      ...
      expect(page.all('.books')[0]).to have_content("#{@book_4.title}")
      expect(page.all('.books')[1]).to have_content("#{@book_5.title}")
      expect(page.all('.books')[2]).to have_content("#{@book_3.title}")
      ...
      expect(page.all('.books')[0]).to have_content("#{@book_5.title}")
      expect(page.all('.books')[1]).to have_content("#{@book_4.title}")
      expect(page.all('.books')[2]).to have_content("#{@book_1.title}")
    end
    it 'I see an area showing statistics about all books' do
      visit books_path
      within("#statistics") do
        expect(page).to have_content("Highest Rated Books: #{@book_4.title}: 5.0 #{@book_5.title}: 4.0 #{@book_3.title}: 3.0")
        expect(page).to have_content("Lowest Rated Books: #{@book_1.title}: 1.0 #{@book_2.title}: 2.0 #{@book_3.title}: 3.0")
        expect(page).to have_content("Most Active Users: David: 5 Joan: 3 Jennica: 2")
      end
    end
  end
